...
export const validateManifest = (manifest) => {
  ...
  for (const target of manifest.targets || []) {
    ...
    if (target.category === 'low_tax') {
      if (!target.tax || typeof target.tax !== 'object' || Array.isArray(target.tax)) {
        errors.push(`${target.id}: low_tax target requires tax metadata`);
      } else {
        if (!taxTypes.has(target.tax.type)) errors.push(`${target.id}: invalid tax type ${target.tax.type || '<blank>'}`);
        if (typeof target.tax.rate !== 'string' || !target.tax.rate.trim().startsWith('≈')) errors.push(`${target.id}: tax rate must be a non-empty string starting with ≈`);
        if (typeof target.tax.label !== 'string' || !target.tax.label.trim()) errors.push(`${target.id}: tax label must be a non-empty string`);
        if (typeof target.tax.note !== 'string') errors.push(`${target.id}: tax note must be a string`);
        if (typeof target.tax.noteZh !== 'string') errors.push(`${target.id}: tax noteZh must be a string`);
      }
    } else if (target.category === 'major_city' && Object.hasOwn(target, 'tax')) {
      errors.push(`${target.id}: major_city target must not define tax metadata`);
    }
...
if (isMain) {
  ...
  if (result.errors.length) {
    ...
    console.log(`[address-lite] config ok countries=${result.countries} targets=${result.targets} groups=${result.groups} theoreticalMaxAddresses=${result.outputCap}`);
  }
}
