...
const dataRoot = resolve(i >= 0 ? args[i + 1] : resolve(root, 'public/data'));
...
for (const country of index.countries) {
  ...
  for (const target of country.targets) {
    ...
    const file = resolve(dataRoot, target.file.replace(/^\/data\//, ''));
...
if (errors.length) {
  ...
  console.log(`[address-lite] static verification passed; addresses=${addresses}`);
}
