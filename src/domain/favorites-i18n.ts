...
export interface FavoritesCopy {
  ...
  position: string; move: string; drag: string; copy: string; copied: string; openGoogle: string; openAmap: string;
}
...
const base: FavoritesCopy = {
  ...
  temporary: 'Browser storage is unavailable. Changes will last for this tab only.', position: 'Position', move: 'Move', drag: 'Drag to reorder', copy: 'Copy', copied: 'Copied', openGoogle: 'Google Maps', openAmap: 'AMap'
};
...
export const favoritesCopy: Record<Locale, FavoritesCopy> = {
  ...
  'zh-CN': {
    ...
    temporary: '浏览器存储不可用，本次修改只在当前标签页保留。', position: '序号', move: '移动', drag: '拖动调整顺序', copy: '复制', copied: '已复制', openGoogle: '谷歌地图', openAmap: '高德地图'
  },
  'zh-TW': {
