...
| 发布门禁 | 质量高于数量。严格高德 L1 可发布；百度/腾讯必须同时通过住宅分类、住宅名称、行政区、数字门牌、坐标和黑名单门禁。关键字段缺失即淘汰；180 天过期即停止发布。 |
| 同步频率 | AreaCity 发布新版时更新行政区；POI 按免费额度分页补齐；短暂限流按平台返回时间恢复，周期额度在重置后自动续跑。单轮遍历全部可用 Key，不设固定重试次数。候选永久保存，发布层可由合格候选重建。 |
| 验证 / 排除 | 任一平台的严格住宅候选可作为 L1；多平台同名、近坐标且门牌一致时升为 L2/L3。门牌或行政区冲突不合并；不使用旧 OSM 中国住宅池。 |
| 策略版本 / 状态 / 更新 | 1.8 / `community-poi-v7` 永久候选层（发布层兼容既有 v6 合格候选）/ 2026-08-02 |
...
- 每个 WebService Key 默认是独立额度；高德默认上限为 `5,000`，名称、密钥、额度上限、日/月周期和启用状态均可在后台编辑。实际周期和上限以用户控制台为准。
- 腾讯返回 `X-Limit` 时显示平台实时已用量与上限；高德、百度响应不提供剩余额度时显示本地统计，并由平台超限错误校正状态。
- 后台显示当前周期已用、上限、剩余、统计来源和重置时间；新增、修改或启用 Key 会唤醒同步。QPS 冷却按服务端 `Retry-After` 或平台错误恢复，额度耗尽后到周期边界自动恢复。
...
- 入库和旧社区读取统一规范 `provider_address`：只移除由至少两个结构化行政字段精确组成的重复前缀，并删除 `西北方向120米` 等导航距离片段；正常道路名不做模糊裁剪。
