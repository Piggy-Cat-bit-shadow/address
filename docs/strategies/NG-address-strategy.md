...
| 项目 | 核心内容 |
|---|---|
| 主源 | Geofabrik `nigeria`；仅在来源版本变化时重新评估 |
...
| 同步频率 | 已停止自动同步；队列固定为 `source_limited/no_source_shard`，除非未来确认新的可发布源。 |
| 验证 / 排除 | 仅发布同时具备完整地址字段和住宅建筑证据的记录；NIPOST 邮编目录只用于邮编核验。 |
| 策略版本 / 状态 / 更新 | 1.5 / 无可发布源，停止自动同步 / 2026-08-03 |
