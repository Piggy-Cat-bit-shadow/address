...
| 初始化 | 一次性读取 17 个 Juso 省级分片，跨分片去重、最低行政区均衡抽样，再与当前住宅建筑相交；Juso 分片最多取 10,000 条，K-apt 官方共同住宅最多取 20,000 条；所有严格记录均可发布，国家完成数量不作为发布上限 |
| 验证 / 排除 | 缺少完整行政层级、门牌、街道、邮编或坐标的记录淘汰；Juso 地址点必须与当前明确住宅建筑相交。 |
| 策略版本 / 状态 / 更新 | 1.13 / 国家完成下限 30,000，Juso 地址存在 + 当前住宅建筑验证 + K-apt 安全部分增量发布 / 2026-08-16 |
