...
| 策略版本 / 状态 / 更新 | 1.8 / 国家完成下限 20,000，Juso 地址存在 + 当前住宅建筑验证 / 2026-08-06 |
...
- 20,000 是国家完成下限而非产量承诺；K-apt 邮编补齐通过本机短生命周期代理逐请求使用后台 Geoapify 凭据池。密钥不会进入 Python 命令行或缓存；认证失败、限速、额度和冷却状态按 Key 持久化，全部不可用时等待最早恢复时间。确定结果写入持久缓存，暂时失败不占用结果缓存；实际请求量由每个凭据及共享额度作用域控制，不再使用单 Key 固定日上限。
